...
public class MyController {
    ...
    @RequestMapping(value = "/delete_photos", method = RequestMethod.POST)
    public String delete(@RequestParam(value = "deletePhotosId[]") long[] delete, Model model) {
        for (long id : delete) {
            photos.remove(id);
        }
        return lookAllPhoto(model);
    }

    @RequestMapping("/photos")
    public String lookAllPhoto(Model model) {
        Set<Long> ids = photos.keySet();
        model.addAttribute("photos_ids", ids);
        return "photos";
    }
